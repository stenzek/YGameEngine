...
class EditorSkeletalMeshEditor : public QMainWindow
{
    Q_OBJECT
...
public:
    EditorSkeletalMeshEditor();
    virtual ~EditorSkeletalMeshEditor();
    ...
    void Create(const char *meshName, ProgressCallbacks *pProgressCallbacks = ProgressCallbacks::NullProgressCallback);
...
private:
    ...
    // --- ui ---
    Ui_EditorSkeletalMeshEditor *m_ui;
    ...
    PODArray<const Material *> m_meshMaterials;

    // --- render mesh ---
    RenderWorld *m_pRenderWorld;
    EditorLightSimulator *m_pLightSimulator;
    CompositeRenderProxy *m_pMeshPreviewRenderProxy;
...
private Q_SLOTS:
    ...
    void OnActionToolImportTriggered(bool checked) { SetCurrentTool((checked) ? Tool_Import : Tool_Information); }
    ...
    void OnToolImportSelectFileNameClicked();
    void OnToolImportImportClicked();
};
