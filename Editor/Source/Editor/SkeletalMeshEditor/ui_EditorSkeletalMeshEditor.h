...
class Ui_EditorSkeletalMeshEditor
{
public:
    ...
    // import
    QWidget *toolImportContainer;
    QPushButton *toolImportSelectFileName;
    QComboBox *toolImportSubMeshName;
    QComboBox *toolImportCoordinateSystem;
    QComboBox *toolImportFaceWinding;
    QCheckBox *toolImportGenerateTangents;
    EditorVectorEditWidget *toolImportScale;
    QCheckBox *toolImportMaterials;
    QLineEdit *toolImportMaterialDirectory;
    QLineEdit *toolImportMaterialPrefix;
    QPushButton *toolImportImport;
    ...
    void CreateUI(QMainWindow *pMainWindow)
    {
        ...
        actionToolImport = new QAction(pMainWindow);
        actionToolImport->setIcon(QIcon(QStringLiteral(":/editor/icons/EditCodeHS.png")));
        actionToolImport->setText(pMainWindow->tr("Import Mesh"));
        actionToolImport->setCheckable(true);
        ...
        menuTools->addAction(actionToolImport);
        ...
        toolbar->addAction(actionToolImport);
        ...
        rightDockContainer = new QWidget(rightDockWidget);
        {
            ...
            // mode toolbar
            {
                ...
                rightToolMenu->addAction(actionToolImport);
            ...
            toolContainer = new QWidget(toolContainerScrollArea);
            {
                ...
                // import tool
                {
                    toolImportContainer = new QWidget(toolContainer);
                    {
                        QFormLayout *formLayout = new QFormLayout(toolImportContainer);
                        formLayout->setLabelAlignment(Qt::AlignTop);

                        toolImportSelectFileName = new QPushButton(pMainWindow->tr("Select File..."), toolImportContainer);
                        formLayout->addRow(toolImportSelectFileName);

                        toolImportSubMeshName = new QComboBox(toolImportContainer);
                        formLayout->addRow(pMainWindow->tr("Sub Mesh: "), toolImportSubMeshName);

                        toolImportCoordinateSystem = new QComboBox(toolImportContainer);
                        toolImportCoordinateSystem->addItem(pMainWindow->tr("Y-Up Left Handed"));
                        toolImportCoordinateSystem->addItem(pMainWindow->tr("Y-Up Right Handed"));
                        toolImportCoordinateSystem->addItem(pMainWindow->tr("Z-Up Left Handed"));
                        toolImportCoordinateSystem->addItem(pMainWindow->tr("Z-Up Right Handed"));
                        formLayout->addRow(pMainWindow->tr("Coordinate System: "), toolImportCoordinateSystem);

                        toolImportFaceWinding = new QComboBox(toolImportContainer);
                        toolImportFaceWinding->addItem(pMainWindow->tr("Counter-Clockwise"));
                        toolImportFaceWinding->addItem(pMainWindow->tr("Clockwise"));
                        formLayout->addRow(pMainWindow->tr("Face Winding: "), toolImportFaceWinding);

                        toolImportGenerateTangents = new QCheckBox(pMainWindow->tr("Generate Tangents"), toolImportContainer);
                        toolImportGenerateTangents->setChecked(true);
                        formLayout->addRow(toolImportGenerateTangents);

                        toolImportScale = new EditorVectorEditWidget(toolImportContainer);
                        toolImportScale->SetNumComponents(3);
                        toolImportScale->SetValue(1.0f, 1.0f, 1.0f);
                        formLayout->addRow(pMainWindow->tr("Scale: "), toolImportScale);

                        toolImportMaterials = new QCheckBox(pMainWindow->tr("Import Materials"), toolImportContainer);
                        toolImportMaterials->setChecked(true);
                        formLayout->addRow(toolImportMaterials);

                        toolImportMaterialDirectory = new QLineEdit(toolImportContainer);
                        formLayout->addRow(pMainWindow->tr("Material Directory"), toolImportMaterialDirectory);

                        toolImportMaterialPrefix = new QLineEdit(toolImportContainer);
                        formLayout->addRow(pMainWindow->tr("Material Prefix"), toolImportMaterialPrefix);

                        toolImportImport = new QPushButton(pMainWindow->tr("Import..."), toolImportContainer);
                        toolImportImport->setEnabled(false);
                        formLayout->addRow(toolImportImport);

                        toolImportContainer->setLayout(formLayout);
                    }
                    toolImportContainer->hide();
                    toolContainerLayout->addWidget(toolImportContainer);
                }
    ...
    void UpdateUIForTool(EditorSkeletalMeshEditor::Tool tool)
    {
        (tool == EditorSkeletalMeshEditor::Tool_Information) ? toolInformationContainer->show() : toolInformationContainer->hide();
        (tool == EditorSkeletalMeshEditor::Tool_Operations) ? toolOperationsContainer->show() : toolOperationsContainer->hide();
        (tool == EditorSkeletalMeshEditor::Tool_Materials) ? toolMaterialsContainer->show() : toolMaterialsContainer->hide();
        (tool == EditorSkeletalMeshEditor::Tool_LOD) ? toolLODContainer->show() : toolLODContainer->hide();
        (tool == EditorSkeletalMeshEditor::Tool_Collision) ? toolCollisionContainer->show() : toolCollisionContainer->hide();
        (tool == EditorSkeletalMeshEditor::Tool_Import) ? toolImportContainer->show() : toolImportContainer->hide();
        (tool == EditorSkeletalMeshEditor::Tool_LightManipulator) ? toolLightManipulatorContainer->show() : toolLightManipulatorContainer->hide();
        actionToolInformation->setChecked((tool == EditorSkeletalMeshEditor::Tool_Information));
        actionToolOperations->setChecked((tool == EditorSkeletalMeshEditor::Tool_Operations));
        actionToolMaterials->setChecked((tool == EditorSkeletalMeshEditor::Tool_Materials));
        actionToolLOD->setChecked((tool == EditorSkeletalMeshEditor::Tool_LOD));
        actionToolCollision->setChecked((tool == EditorSkeletalMeshEditor::Tool_Collision));
        actionToolImport->setChecked((tool == EditorSkeletalMeshEditor::Tool_Import));
        actionToolLightManipulator->setChecked((tool == EditorSkeletalMeshEditor::Tool_LightManipulator));
    }
};
