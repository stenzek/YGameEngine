    ...
    , m_pConstants(nullptr)
{
    ...
    m_pUserIndexBuffer = NULL;
    m_userIndexBufferSize = 512 * 1024;
    m_userIndexBufferPosition = 0;
}
...
bool D3D11GPUContext::GetTempVertexBufferPointer(uint32 VertexSize, uint32 nVertices, void **ppBufferPointer, uint32 *pBufferOffset, uint32 *pMaxVertices)
{
    ...
    // return results
    *ppBufferPointer = reinterpret_cast<void *>(reinterpret_cast<byte *>(mappedSubResource.pData) + m_userVertexBufferPosition);
    *pBufferOffset = m_userVertexBufferPosition;
    *pMaxVertices = maxVertices;
...
bool D3D11GPUContext::GetTempIndexBufferPointer(uint32 indexSize, uint32 nIndices, void **ppBufferPointer, uint32 *pBufferOffset, uint32 *pMaxIndices)
{
    uint32 maxIndices = Min(nIndices, (m_userVertexBufferSize / indexSize));
    uint32 requestedSpace = maxIndices * indexSize;
    DebugAssert(maxIndices > 0);

    HRESULT hResult;
    D3D11_MAPPED_SUBRESOURCE mappedSubResource;
    if ((m_userIndexBufferPosition + requestedSpace) < m_userVertexBufferSize)
    {
        // fit into remaining space
        hResult = m_pD3DContext->Map(m_pUserIndexBuffer, 0, D3D11_MAP_WRITE_NO_OVERWRITE, 0, &mappedSubResource);
    }
    else
    {
        // discard all of the buffer
        m_userIndexBufferPosition = 0;
        hResult = m_pD3DContext->Map(m_pUserIndexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubResource);
    }
    ...
    // return results
    *ppBufferPointer = reinterpret_cast<void *>(reinterpret_cast<byte *>(mappedSubResource.pData) + m_userIndexBufferPosition);
    *pBufferOffset = m_currentIndexBufferOffset;
    *pMaxIndices = maxIndices;

    // increment positions
    m_userIndexBufferSize += requestedSpace;
    return true;
}
...
//     if (m_currentVertexBufferBindCount > 0)
//     {
//         ID3D11Buffer *pNullD3DBuffers[GPU_MAX_SIMULTANEOUS_VERTEX_BUFFERS] = { NULL };
...
//         ID3D11Buffer *pD3DBuffers[GPU_MAX_SIMULTANEOUS_VERTEX_BUFFERS];
...
void D3D11GPUContext::DrawIndexedUserPointer(const void *pVertices, uint32 vertexSize, uint32 nVertices, const void *pIndices, GPU_INDEX_FORMAT indexFormat, uint32 nIndices)
{
    uint32 i;
    void *pMappedPointer;
    ...
    uint32 mappedVertices;
    if (!GetTempVertexBufferPointer(vertexSize, nVertices, &pMappedPointer, &vertexBufferOffset, &mappedVertices))
        return;
    if (mappedVertices != nVertices)
    {
        Log_ErrorPrintf("D3D11GPUDevice::DrawIndexedUserPointer: Not enough space (%u vertices required, only fit %u)", nVertices, mappedVertices);
        m_pD3DContext->Unmap(m_pUserVertexBuffer, 0);
        return;
    }

    // write it
    Y_memcpy(pMappedPointer, pVertices, vertexSize * mappedVertices);
    m_pD3DContext->Unmap(m_pUserVertexBuffer, 0);

    // map index buffer
    uint32 indexSize = (indexFormat == GPU_INDEX_FORMAT_UINT32) ? sizeof(uint32) : sizeof(uint16);
    uint32 indexBufferOffset;
    uint32 mappedIndices;
    if (!GetTempIndexBufferPointer(indexSize, nIndices, &pMappedPointer, &indexBufferOffset, &mappedIndices))
        return;
    if (mappedIndices != nIndices)
    {
        Log_ErrorPrintf("D3D11GPUDevice::DrawIndexedUserPointer: Not enough space (%u indices required, only fit %u)", nIndices, mappedIndices);
        m_pD3DContext->Unmap(m_pUserIndexBuffer, 0);
        return;
    }

    // write it
    Y_memcpy(pMappedPointer, pIndices, indexSize * mappedIndices);
    m_pD3DContext->Unmap(m_pUserIndexBuffer, 0);
    ...
    if (m_pCurrentIndexBuffer != NULL)
        m_pD3DContext->IASetIndexBuffer(NULL, DXGI_FORMAT_UNKNOWN, 0);

    // bind buffers
    m_pD3DContext->IASetVertexBuffers(0, 1, &m_pUserVertexBuffer, &vertexSize, &vertexBufferOffset);
    m_pD3DContext->IASetIndexBuffer(m_pUserIndexBuffer, (indexFormat == GPU_INDEX_FORMAT_UINT32) ? DXGI_FORMAT_R32_UINT : DXGI_FORMAT_R16_UINT, indexBufferOffset);
    ...
    if (m_currentVertexBufferBindCount > 0)
    {
        ...
        UINT D3DOffsets[GPU_MAX_SIMULTANEOUS_VERTEX_BUFFERS];
        UINT D3DStrides[GPU_MAX_SIMULTANEOUS_VERTEX_BUFFERS];
        for (i = 0; i < m_currentVertexBufferBindCount; i++)
        {
            pD3DBuffers[i] = (m_pCurrentVertexBuffers[i] != NULL) ? m_pCurrentVertexBuffers[i]->GetD3DBuffer() : NULL;
            D3DOffsets[i] = m_currentVertexBufferOffsets[i];
            D3DStrides[i] = m_currentVertexBufferStrides[i];
    ...
    if (m_pCurrentIndexBuffer != NULL)
        m_pD3DContext->IASetIndexBuffer(m_pCurrentIndexBuffer->GetD3DBuffer(), (m_currentIndexFormat == GPU_INDEX_FORMAT_UINT16) ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT, m_currentIndexBufferOffset);
    else
        m_pD3DContext->IASetIndexBuffer(NULL, DXGI_FORMAT_UNKNOWN, 0);
}
