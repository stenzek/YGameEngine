...
D3D11RendererOutputBuffer::D3D11RendererOutputBuffer(ID3D11Device *pD3DDevice, IDXGISwapChain *pDXGISwapChain, HWND hWnd, uint32 width, uint32 height, DXGI_FORMAT backBufferFormat, DXGI_FORMAT depthStencilBufferFormat, RENDERER_VSYNC_TYPE vsyncType)
...
D3D11RendererOutputBuffer::~D3D11RendererOutputBuffer()
{
...
D3D11RendererOutputBuffer *D3D11RendererOutputBuffer::Create(IDXGIFactory *pDXGIFactory, ID3D11Device *pD3DDevice, HWND hWnd, DXGI_FORMAT backBufferFormat, DXGI_FORMAT depthStencilBufferFormat, RENDERER_VSYNC_TYPE vsyncType)
{
    ...
    if (FAILED(hResult))
    {
        Log_ErrorPrintf("D3D11RendererOutputBuffer::Create: CreateSwapChain failed with hResult %08X", hResult);
        return false;
    }

    // create object
    pD3DDevice->AddRef();
    D3D11RendererOutputBuffer *pSwapChain = new D3D11RendererOutputBuffer(pD3DDevice, pDXGISwapChain, hWnd, width, height, backBufferFormat, depthStencilBufferFormat, vsyncType);
    if (!pSwapChain->InternalCreateBuffers())
    {
        Log_ErrorPrintf("D3D11RendererOutputBuffer::Create: CreateRTVAndDepthStencilBuffer failed");
        pSwapChain->Release();
        return NULL;
    }
...
void D3D11RendererOutputBuffer::InternalResizeBuffers(uint32 width, uint32 height, RENDERER_VSYNC_TYPE vsyncType)
{
    HRESULT hResult;
    ...
    Log_DevPrintf("D3D11RendererOutputBuffer::InternalResizeBuffers: New buffer count = %u", bufferCount);
    ...
    if (FAILED(hResult))
        Panic("D3D11RendererOutputBuffer::ResizeBuffers: IDXGISwapChain::ResizeBuffers failed.");
    ...
    if (!InternalCreateBuffers())
        Panic("D3D11RendererOutputBuffer::ResizeBuffers: Failed to recreate texture objects on resized swap chain.");
}


bool D3D11RendererOutputBuffer::InternalCreateBuffers()
{
    ...
    if (FAILED(hResult))
    {
        Log_ErrorPrintf("D3D11RendererOutputBuffer::CreateRTVAndDepthStencilBuffer: IDXGISwapChain::GetBuffer failed with hResult %08X", hResult);
        return false;
    }
    ...
    if (FAILED(hResult))
    {
        Log_ErrorPrintf("D3D11RendererOutputBuffer::CreateRTVAndDepthStencilBuffer: CreateRenderTargetView failed with hResult %08X", hResult);
    ...
    if (m_depthStencilBufferFormat != DXGI_FORMAT_UNKNOWN)
    {
        ...
        if (FAILED(hResult))
        {
            Log_ErrorPrintf("D3D11RendererOutputBuffer::CreateRTVAndDepthStencilBuffer: CreateTexture2D for DS failed with hResult %08X", hResult);
        ...
        if (FAILED(hResult))
        {
            Log_ErrorPrintf("D3D11RendererOutputBuffer::CreateRTVAndDepthStencilBuffer: CreateDepthStencilView failed with hResult %08X", hResult);
...
void D3D11RendererOutputBuffer::InternalReleaseBuffers()
{
...
bool D3D11RendererOutputBuffer::ResizeBuffers(uint32 width /* = 0 */, uint32 height /* = 0 */)
{
    if (width == 0 || height == 0)
    {
...
void D3D11RendererOutputBuffer::SetVSyncType(RENDERER_VSYNC_TYPE vsyncType)
{
...
void D3D11RendererOutputBuffer::SwapBuffers()
{
...
D3D11RendererOutputWindow::D3D11RendererOutputWindow(SDL_Window *pSDLWindow, D3D11RendererOutputBuffer *pBuffer, RENDERER_FULLSCREEN_STATE fullscreenState)
...
D3D11RendererOutputWindow::~D3D11RendererOutputWindow()
{
...
D3D11RendererOutputWindow *D3D11RendererOutputWindow::Create(IDXGIFactory *pDXGIFactory, IDXGIAdapter *pDXGIAdapter, ID3D11Device *pD3DDevice, const char *windowCaption, uint32 windowWidth, uint32 windowHeight, DXGI_FORMAT backBufferFormat, DXGI_FORMAT depthStencilBufferFormat, RENDERER_VSYNC_TYPE vsyncType, bool visible)
{
    ...
    {
        Log_ErrorPrintf("D3D11RendererOutputWindow::Create: SDL_CreateWindow failed: %s", SDL_GetError());
        return nullptr;
    }
    ...
    {
        Log_ErrorPrintf("D3D11RendererOutputWindow::Create: SDL_GetWindowWMInfo failed: %s", SDL_GetError());
    ...
    if (info.subsystem == SDL_SYSWM_WINDOWS)
    {
        ...
        if (FAILED(hResult))
        {
            Log_ErrorPrintf("D3D11RendererOutputWindow::Create: CreateSwapChain failed with hResult %08X.", hResult);
            SDL_DestroyWindow(pSDLWindow);
            return nullptr;
        }
        ...
        if (FAILED(hResult))
        {
            Log_ErrorPrintf("D3D11RendererOutputWindow::Create: MakeWindowAssociation failed with hResult %08X.", hResult);
    ...
    {
        Log_ErrorPrintf("D3D11RendererOutputWindow::Create: Unhandled syswm: %u", (uint32)info.subsystem);
        SDL_DestroyWindow(pSDLWindow);
        return nullptr;
    }

    // create the buffer
    D3D11RendererOutputBuffer *pOutputBuffer;
    {
        pD3DDevice->AddRef();
        pOutputBuffer = new D3D11RendererOutputBuffer(pD3DDevice, pDXGISwapChain, info.info.win.window, windowWidth, windowHeight, backBufferFormat, depthStencilBufferFormat, vsyncType);
    ...
    D3D11RendererOutputWindow *pOutputWindow = new D3D11RendererOutputWindow(pSDLWindow, pOutputBuffer, RENDERER_FULLSCREEN_STATE_WINDOWED);
...
bool D3D11RendererOutputWindow::SetFullScreen(RENDERER_FULLSCREEN_STATE state, uint32 width /* = 0 */, uint32 height /* = 0 */)
{
    ...
    if (state == RENDERER_FULLSCREEN_STATE_FULLSCREEN)
    {
        ...
        // get output
        IDXGIOutput *pOutput;
        hResult = pGPUContext->GetRenderer()->GetDXGIAdapter()->EnumOutputs(0, &pOutput);
        if (SUCCEEDED(hResult))
        {
            ...
            hResult = pOutput->FindClosestMatchingMode(&modeDesc, &closestMatch, pGPUContext->GetRenderer()->GetD3DDevice());
            if (SUCCEEDED(hResult))
            {
                // update dimensions with closest match
                Log_InfoPrintf("D3D11RendererOutputWindow::SetFullScreen: Switching to closest mode match %ux%u RefreshRate %u/%u", closestMatch.Width, closestMatch.Height, closestMatch.RefreshRate.Numerator, closestMatch.RefreshRate.Denominator);
                ...
                if (m_fullscreenState != RENDERER_FULLSCREEN_STATE_FULLSCREEN)
                {
                    ...
                    if (FAILED(hResult))
                    {
                        Log_ErrorPrintf("D3D11RendererOutputWindow::SetFullScreen: IDXGISwapChain::SetFullscreenState failed with hResult %08X.", hResult);
                        switchResult = false;
                    }
                }
                ...
                if (switchResult)
                {
                    ...
                    if (FAILED(hResult))
                    {
                        Log_ErrorPrintf("D3D11GPUManagedSwapChain::SetFullScreen: IDXGISwapChain::ResizeTarget failed with hResult %08X.", hResult);
                        switchResult = false;
                    }
                }
            }
            ...
            {
                Log_ErrorPrintf("D3D11GPUManagedSwapChain::SetFullScreen: IDXGIOutput::FindClosestMatchingMode failed with hResult %08X.", hResult);
                switchResult = false;
            }
        ...
        {
            Log_ErrorPrintf("D3D11GPUManagedSwapChain::SetFullScreen: IDXGIAdapter::EnumOutputs failed with hResult %08X.", hResult);
            switchResult = false;
        }
    }
    else
...
int D3D11RendererOutputWindow::PendingSDLEventFilter(void *pUserData, SDL_Event *pEvent)
{
    switch (pEvent->type)
    {
...
void D3D11Renderer::HandlePendingSDLEvents()
{
    SDL_FilterEvents(&D3D11RendererOutputWindow::PendingSDLEventFilter, nullptr);
}

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

RendererOutputBuffer *D3D11Renderer::CreateOutputBuffer(RenderSystemWindowHandle hWnd, RENDERER_VSYNC_TYPE vsyncType)
{
    return D3D11RendererOutputBuffer::Create(m_pDXGIFactory, m_pD3DDevice, hWnd, m_swapChainBackBufferFormat, m_swapChainDepthStencilBufferFormat, vsyncType);
}

RendererOutputWindow *D3D11Renderer::CreateOutputWindow(const char *windowTitle, uint32 windowWidth, uint32 windowHeight, RENDERER_VSYNC_TYPE vsyncType)
{
