...
Log_SetChannel(OpenGLES2RendererOutputBuffer);
...
OpenGLES2RendererOutputBuffer::~OpenGLES2RendererOutputBuffer()
{
...
OpenGLES2RendererOutputBuffer *OpenGLES2RendererOutputBuffer::Create(RenderSystemWindowHandle windowHandle, PIXEL_FORMAT backBufferFormat, PIXEL_FORMAT depthStencilBufferFormat, RENDERER_VSYNC_TYPE vsyncType)
{
    ...
    {
        Log_ErrorPrintf("OpenGLES2RendererOutputBuffer::Create: SDL_CreateWindowFrom failed: %s", SDL_GetError());
        return nullptr;
    }
...
OpenGLES2RendererOutputWindow::OpenGLES2RendererOutputWindow(SDL_Window *pSDLWindow, OpenGLES2RendererOutputBuffer *pBuffer, RENDERER_FULLSCREEN_STATE fullscreenState)
...
OpenGLES2RendererOutputWindow::~OpenGLES2RendererOutputWindow()
{
...
OpenGLES2RendererOutputWindow *OpenGLES2RendererOutputWindow::Create(const char *windowCaption, uint32 windowWidth, uint32 windowHeight, PIXEL_FORMAT backBufferFormat, PIXEL_FORMAT depthStencilBufferFormat, RENDERER_VSYNC_TYPE vsyncType, bool visible)
{
    ...
    if (pSDLWindow == nullptr)
    {
        Log_ErrorPrintf("OpenGLES2RendererOutputWindow::Create: SDL_CreateWindow failed: %s", SDL_GetError());
        return nullptr;
    }

    // create the buffer
    OpenGLES2RendererOutputBuffer *pOutputBuffer = new OpenGLES2RendererOutputBuffer(pSDLWindow, false, windowWidth, windowHeight, backBufferFormat, depthStencilBufferFormat, vsyncType);

    // create the output window
    OpenGLES2RendererOutputWindow *pOutputWindow = new OpenGLES2RendererOutputWindow(pSDLWindow, pOutputBuffer, RENDERER_FULLSCREEN_STATE_WINDOWED);
...
bool OpenGLES2RendererOutputWindow::SetFullScreen(RENDERER_FULLSCREEN_STATE state, uint32 width /* = 0 */, uint32 height /* = 0 */)
{
    if (state == RENDERER_FULLSCREEN_STATE_FULLSCREEN)
    {
        ...
        default:
            Log_ErrorPrintf("OpenGLES2RendererOutputWindow::SetFullScreen: Unable to find SDL pixel format for %s", PixelFormat_GetPixelFormatInfo(m_pOpenGLOutputBuffer->GetBackBufferFormat())->Name);
            return false;
        }
        ...
        {
            Log_ErrorPrintf("OpenGLES2RendererOutputWindow::SetFullScreen: Unable to find matching video mode for: %i x %i (%s)", preferredDisplayMode.w, preferredDisplayMode.h, PixelFormat_GetPixelFormatInfo(m_pOpenGLOutputBuffer->GetBackBufferFormat())->Name);
            return false;
        }
        ...
        {
            Log_ErrorPrintf("OpenGLES2RendererOutputWindow::SetFullScreen: SDL_SetWindowDisplayMode failed: %s", SDL_GetError());
            return false;
        }
        ...
        if (m_fullscreenState != RENDERER_FULLSCREEN_STATE_FULLSCREEN)
        {
            ...
            {
                Log_ErrorPrintf("OpenGLES2RendererOutputWindow::SetFullScreen: SDL_SetWindowFullscreen failed: %s", SDL_GetError());
                return false;
            }
...
int OpenGLES2RendererOutputWindow::PendingSDLEventFilter(void *pUserData, SDL_Event *pEvent)
{
//     switch (pEvent->type)
//     {
...
void OpenGLES2Renderer::HandlePendingSDLEvents()
{
    SDL_FilterEvents(&OpenGLES2RendererOutputWindow::PendingSDLEventFilter, nullptr);
}
...
RendererOutputWindow *OpenGLES2Renderer::CreateOutputWindow(const char *windowTitle, uint32 windowWidth, uint32 windowHeight, RENDERER_VSYNC_TYPE vsyncType)
{
