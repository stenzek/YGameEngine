...
void TextureBlitShader::SetProgramParameters(GPUCommandList *pCommandList, ShaderProgram *pShaderProgram, GPUTexture *pSourceTexture, GPUSamplerState *pSamplerState, uint32 sourceLevel)
{
    float fSourceLevel = (float)sourceLevel;
    pShaderProgram->SetBaseShaderParameterTexture(pCommandList, 0, pSourceTexture, pSamplerState);
    pShaderProgram->SetBaseShaderParameterValue(pCommandList, 1, SHADER_PARAMETER_TYPE_FLOAT, &fSourceLevel);
}
